package RTDRestaurant.Controller.Service.Admin;
...
public class ServiceAdmin {
    ...
    public ArrayList<ModelChart> getRevenueCostProfit_byMonth() throws SQLException{
        ...
        String sql_Cost = "SELECT EXTRACT(MONTH FROM NgayNK) as Thang, SUM(TONGTIEN) FROM PhieuNK WHERE EXTRACT(YEAR FROM NgayNK)=EXTRACT(YEAR FROM CURRENT_DATE) "
                + "GROUP BY EXTRACT(MONTH FROM NgayNK)";

        Map<Integer, Integer> revenueByMonth = new HashMap<>();
        Map<Integer, Integer> costByMonth = new HashMap<>();

        try (PreparedStatement pR = con.prepareStatement(sql_Revenue);
                ResultSet rR = pR.executeQuery()) {
            while (rR.next()) {
                int month = rR.getInt(1);
                int revenue = rR.getInt(2);
                revenueByMonth.put(month, revenue);
            }
        }

        try (PreparedStatement pC = con.prepareStatement(sql_Cost);
                ResultSet rC = pC.executeQuery()) {
            while (rC.next()) {
                int month = rC.getInt(1);
                int cost = rC.getInt(2);
                costByMonth.put(month, cost);
            }
        }

        int maxMonth = 0;
        for (Integer month : revenueByMonth.keySet()) {
            if (month > maxMonth) {
                maxMonth = month;
            }
        }
        for (Integer month : costByMonth.keySet()) {
            if (month > maxMonth) {
                maxMonth = month;
            }
        }
        ...
        for (int month = 1; month <= maxMonth; month++) {
            int revenue = revenueByMonth.getOrDefault(month, 0);
            int expenses = costByMonth.getOrDefault(month, 0);
            int profit = revenue - expenses;
            list.add(new ModelChart("Tháng " + month, new double[]{revenue, expenses, profit}));
        }
        return list;
    }
