...
import RTDRestaurant.Controller.Service.Customer.ServiceCustomer;
    ...
    public CardVoucher(ModelVoucher data,ModelKhachHang customer,ModelHoaDon hoadon) {
        ...
        cmdExchange.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if(hoadon==null){
                    // Đổi voucher trực tiếp (không cần hóa đơn)
                    if(customer.getPoints()<Vpoint){
                        warning.WarningExchange(Vpoint-customer.getPoints());
                    }else{
                        try {
                            service.exchangeVoucherDirect(customer.getID_KH(), data.getCode());
                            // Hiển thị dialog xác nhận thành công
                            warning.ShowMessage("Đổi Voucher Thành Công", "Bạn đã đổi Voucher thành công!");
                        } catch (SQLException ex) {
                            Logger.getLogger(CardVoucher.class.getName()).log(Level.SEVERE, null, ex);
                            String errMsg = ex.getMessage();
                            if(errMsg != null && errMsg.contains("Khong du diem")) {
                                warning.ShowMessage("Không Đủ Điểm", "Bạn không đủ điểm tích lũy để đổi Voucher này.");
                            } else if(errMsg != null && errMsg.contains("khong ton tai")) {
                                warning.ShowMessage("Lỗi", "Voucher không tồn tại hoặc đã hết số lượng.");
                            } else {
                                warning.ShowMessage("Lỗi", "Đổi voucher thất bại: " + (errMsg != null ? errMsg : "Lỗi không xác định"));
                            }
                        }
                    }
                }
