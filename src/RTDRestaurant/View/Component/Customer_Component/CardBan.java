...
public class CardBan extends javax.swing.JPanel {
    ...
    private final MS_Confirm confirmDialog;

    public CardBan(ModelBan data, ModelKhachHang customer) {
        this.data = data;
        this.customer = customer;
        this.confirmDialog = new MS_Confirm(Main_Customer_Frame.getFrames()[0], true);
        initComponents();
        configureCardState();
        setOpaque(false);
    }

    private void configureCardState() {
        ...
        switch (data.getStatus()) {
            case "Con trong" -> {
                ...
                cmdBook.addActionListener(e -> confirmDialog.Cus_ConfirmBook(data, customer));
            }
            case "Dang dung bua" -> {
