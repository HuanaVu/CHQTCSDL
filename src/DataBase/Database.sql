-- Royal Restaurant Management System Database Schema
-- Last Updated: April 2026
-- 
-- PAYMENT SYSTEM UPDATE:
-- Added PaymentMethod (cash|card|transfer) and PaymentRequestStatus (pending|approved) columns to HoaDon table
-- These columns support the new payment approval workflow:
-- - Customers select payment method when creating payment request (request-pay action)
-- - Admin can approve payment requests (confirm-pay action)
-- - Payment status transitions: Chua thanh toan -> Da thanh toan
-- 
-- Sample Data:
-- - Bills 101-105, 109-110, 113, 116, 118-119, 121-122: Already paid (Da thanh toan with PaymentMethod)
-- - Bills 106, 111, 114: Pending payment approval (Chua thanh toan with PaymentRequestStatus=pending)
-- - Bills 107-108, 112, 115, 117, 120: Unpaid without payment request (Chua thanh toan with NULL statuses)
--
...
create table HoaDon(
    ...
    Trangthai varchar2(50),
    PaymentMethod varchar2(20) DEFAULT NULL,
    PaymentRequestStatus varchar2(20) DEFAULT NULL
);
...
alter table HoaDon
    ...
    add constraint HD_PaymentMethod check (PaymentMethod is null or PaymentMethod in ('cash','card','transfer'))
    add constraint HD_PaymentRequestStatus check (PaymentRequestStatus is null or PaymentRequestStatus in ('pending','approved'));
...
-- Khach hang khong duoc dat 2 ban cung luc va moi ban chi co toi da 1 hoa don Chua thanh toan
-- Dung compound trigger de tranh loi mutating table khi kiem tra tren bang HoaDon
CREATE OR REPLACE TRIGGER Tg_SLHD_CTT
FOR INSERT OR UPDATE OF ID_KH, ID_Ban, TrangThai ON HoaDon
COMPOUND TRIGGER

    TYPE t_id_list IS TABLE OF NUMBER INDEX BY PLS_INTEGER;
    g_customer_ids t_id_list;
    g_table_ids t_id_list;
    g_customer_count PLS_INTEGER := 0;
    g_table_count PLS_INTEGER := 0;

    PROCEDURE add_customer_id(p_id NUMBER) IS
    BEGIN
        IF p_id IS NULL THEN
            RETURN;
        END IF;
        g_customer_count := g_customer_count + 1;
        g_customer_ids(g_customer_count) := p_id;
    END;

    PROCEDURE add_table_id(p_id NUMBER) IS
    BEGIN
        IF p_id IS NULL THEN
            RETURN;
        END IF;
        g_table_count := g_table_count + 1;
        g_table_ids(g_table_count) := p_id;
    END;

    BEFORE EACH ROW IS
    BEGIN
        IF :new.TrangThai = 'Chua thanh toan' THEN
            add_customer_id(:new.ID_KH);
            add_table_id(:new.ID_Ban);
        END IF;

        IF UPDATING AND :old.TrangThai = 'Chua thanh toan' THEN
            add_customer_id(:old.ID_KH);
            add_table_id(:old.ID_Ban);
        END IF;
    END BEFORE EACH ROW;
    ...
    AFTER STATEMENT IS
        v_count NUMBER;
    BEGIN
        FOR i IN 1 .. g_customer_count LOOP
            SELECT COUNT(*)
            INTO v_count
            FROM HoaDon
            WHERE ID_KH = g_customer_ids(i)
              AND TrangThai = 'Chua thanh toan';

            IF v_count > 1 THEN
                RAISE_APPLICATION_ERROR(-20001, 'Moi khach hang chi duoc dat 1 ban tai 1 thoi diem');
            END IF;
        END LOOP;

        FOR i IN 1 .. g_table_count LOOP
            SELECT COUNT(*)
            INTO v_count
            FROM HoaDon
            WHERE ID_Ban = g_table_ids(i)
              AND TrangThai = 'Chua thanh toan';

            IF v_count > 1 THEN
                RAISE_APPLICATION_ERROR(-20002, 'Ban nay da co hoa don chua thanh toan');
            END IF;
        END LOOP;
    END AFTER STATEMENT;
END Tg_SLHD_CTT;
...
FOR EACH ROW
BEGIN
    IF :new.Trangthai='Da thanh toan' AND NVL(:old.Trangthai, 'x') <> 'Da thanh toan' THEN
        UPDATE KhachHang
        SET Doanhso = NVL(Doanhso, 0) + NVL(:new.Tongtien, 0)
        WHERE ID_KH=:new.ID_KH;

        UPDATE KhachHang
        SET Diemtichluy = NVL(Diemtichluy, 0) + ROUND(NVL(:new.Tongtien, 0) * 0.00005)
        WHERE ID_KH=:new.ID_KH;
    END IF;
END;
/
...
-- Procedure doi Voucher truc tiep (khong can Hoa Don)
-- Kiem tra diem du, tru diem va giam so luong Voucher
CREATE OR REPLACE PROCEDURE exchangeVoucherDirect(p_ID_KH KHACHHANG.ID_KH%TYPE, p_Code_Voucher Voucher.Code_Voucher%TYPE)
IS
    v_diemdoi NUMBER;
    v_diemkh NUMBER;
    v_count NUMBER;
BEGIN
    ...
    SELECT COUNT(*), NVL(Diem, 0)
    INTO v_count, v_diemdoi
    FROM Voucher
    WHERE Code_Voucher = p_Code_Voucher;
    
    IF v_count = 0 THEN
        RAISE_APPLICATION_ERROR(-20025, 'Voucher khong ton tai');
    END IF;
    
    -- Kiem tra khach hang co ton tai khong va lay so diem hien co
    SELECT COUNT(*), NVL(Diemtichluy, 0)
    INTO v_count, v_diemkh
    FROM KHACHHANG
    WHERE ID_KH = p_ID_KH;
    ...
    -- Kiem tra diem du khong
    IF v_diemkh < v_diemdoi THEN
        RAISE_APPLICATION_ERROR(-20026, 'Khong du diem tich luy. Can them ' || (v_diemdoi - v_diemkh) || ' xu');
    END IF;
    
    -- Tru diem tich luy
    KH_TruDTL(p_ID_KH, v_diemdoi);
    
    -- Giam so luong voucher
    Voucher_GiamSL(p_Code_Voucher);
...
-- =====================================================
-- Web helper functions/procedures/triggers (Oracle)
-- =====================================================

-- Kiem tra 1 ban co hoa don chua thanh toan hay khong
CREATE OR REPLACE FUNCTION WEB_HAS_UNPAID_BILL_BY_TABLE(p_id_ban NUMBER)
RETURN NUMBER
IS
    v_count NUMBER;
BEGIN
...
-- Kiem tra 1 khach co hoa don chua thanh toan hay khong
CREATE OR REPLACE FUNCTION WEB_HAS_UNPAID_BILL_BY_CUSTOMER(p_id_kh NUMBER)
RETURN NUMBER
IS
    v_count NUMBER;
BEGIN
...
-- Kiem tra co the dat ban hay khong (1=co, 0=khong)
CREATE OR REPLACE FUNCTION WEB_CAN_BOOK_TABLE(p_id_kh NUMBER, p_id_ban NUMBER)
RETURN NUMBER
IS
    v_table_status Ban.Trangthai%TYPE;
BEGIN
    IF WEB_HAS_UNPAID_BILL_BY_CUSTOMER(p_id_kh) = 1 THEN
        RETURN 0;
    END IF;

    IF WEB_HAS_UNPAID_BILL_BY_TABLE(p_id_ban) = 1 THEN
        RETURN 0;
    END IF;
    ...
    IF v_table_status = 'Con trong' THEN
        RETURN 1;
    END IF;
...
-- Dong bo trang thai tat ca ban voi du lieu hoa don
-- p_release_reserved = 1: neu ban khong con hoa don chua thanh toan thi dua ve Con trong
CREATE OR REPLACE PROCEDURE WEB_SYNC_TABLE_STATUS(p_release_reserved NUMBER DEFAULT 0)
IS
BEGIN
    UPDATE Ban b
    SET Trangthai = CASE
        WHEN EXISTS (
            ...
            WHERE h.ID_Ban = b.ID_Ban
              AND h.Trangthai = 'Chua thanh toan'
        ) THEN 'Dang dung bua'
        WHEN p_release_reserved = 1 AND b.Trangthai IN ('Dang dung bua', 'Da dat truoc') THEN 'Con trong'
        WHEN b.Trangthai = 'Dang dung bua' THEN 'Con trong'
        ELSE b.Trangthai
    END;
END;
/

-- Thanh toan hoa don theo luong web va tra ban ve Con trong
CREATE OR REPLACE PROCEDURE WEB_MARK_BILL_PAID(
    p_bill_id IN NUMBER,
    p_payment_method IN VARCHAR2 DEFAULT 'cash'
)
IS
    v_status HoaDon.Trangthai%TYPE;
    v_has_payment_method NUMBER := 0;
    v_has_payment_request_status NUMBER := 0;
    v_sql VARCHAR2(500);
BEGIN
    SELECT Trangthai
    INTO v_status
    FROM HoaDon
    WHERE ID_HoaDon = p_bill_id
    FOR UPDATE;

    IF v_status = 'Da thanh toan' THEN
        RETURN;
    END IF;
    ...
    SELECT COUNT(*)
    INTO v_has_payment_request_status
    FROM USER_TAB_COLUMNS
    WHERE TABLE_NAME = 'HOADON'
      AND COLUMN_NAME = 'PAYMENTREQUESTSTATUS';

    v_sql := 'UPDATE HoaDon SET Trangthai = ''Da thanh toan''';
    IF v_has_payment_method > 0 THEN
        v_sql := v_sql || ', PaymentMethod = ''cash''';
    END IF;
    IF v_has_payment_request_status > 0 THEN
        v_sql := v_sql || ', PaymentRequestStatus = ''approved''';
    END IF;
    v_sql := v_sql || ' WHERE ID_HoaDon = :1';
    EXECUTE IMMEDIATE v_sql USING p_bill_id;

    UPDATE Ban
    SET Trangthai = 'Con trong'
    WHERE ID_Ban = (SELECT ID_Ban FROM HoaDon WHERE ID_HoaDon = p_bill_id);
END;
/

-- Trigger bao ve luong trang thai thanh toan cho web
CREATE OR REPLACE TRIGGER TG_HD_WEB_VALIDATE_PAYFLOW
BEFORE INSERT OR UPDATE OF Trangthai ON HoaDon
FOR EACH ROW
BEGIN
    IF UPDATING THEN
        IF :old.Trangthai = 'Da thanh toan' AND :new.Trangthai = 'Chua thanh toan' THEN
            RAISE_APPLICATION_ERROR(-20021, 'Khong duoc chuyen hoa don da thanh toan ve chua thanh toan');
        END IF;
    END IF;
END;
/

-- Trigger khong cho diem tich luy am
CREATE OR REPLACE TRIGGER TG_KH_WEB_NO_NEGATIVE_POINTS
BEFORE INSERT OR UPDATE OF Diemtichluy ON KhachHang
FOR EACH ROW
BEGIN
    IF NVL(:new.Diemtichluy, 0) < 0 THEN
        RAISE_APPLICATION_ERROR(-20023, 'Diem tich luy khong duoc am');
    END IF;
END;
/

-- Trigger khong cho so luong voucher am
CREATE OR REPLACE TRIGGER TG_VOUCHER_WEB_NO_NEGATIVE_SL
BEFORE INSERT OR UPDATE OF SoLuong ON Voucher
FOR EACH ROW
BEGIN
    IF NVL(:new.SoLuong, 0) < 0 THEN
        RAISE_APPLICATION_ERROR(-20024, 'So luong voucher khong duoc am');
    END IF;
END;
/

-- Trigger chong SoLuong <= 0 cho cac bang chi tiet
CREATE OR REPLACE TRIGGER TG_CTHD_WEB_POSITIVE_QTY
BEFORE INSERT OR UPDATE OF SoLuong ON CTHD
FOR EACH ROW
BEGIN
    IF NVL(:new.SoLuong, 0) <= 0 THEN
        RAISE_APPLICATION_ERROR(-20025, 'So luong CTHD phai lon hon 0');
    END IF;
END;
/

CREATE OR REPLACE TRIGGER TG_CTNK_WEB_POSITIVE_QTY
BEFORE INSERT OR UPDATE OF SoLuong ON CTNK
FOR EACH ROW
BEGIN
    IF NVL(:new.SoLuong, 0) <= 0 THEN
        RAISE_APPLICATION_ERROR(-20026, 'So luong CTNK phai lon hon 0');
    END IF;
END;
/

CREATE OR REPLACE TRIGGER TG_CTXK_WEB_POSITIVE_QTY
BEFORE INSERT OR UPDATE OF SoLuong ON CTXK
FOR EACH ROW
BEGIN
    IF NVL(:new.SoLuong, 0) <= 0 THEN
        RAISE_APPLICATION_ERROR(-20027, 'So luong CTXK phai lon hon 0');
    END IF;
END;
/

-- Procedure tinh lai tien mon an/tong tien cho 1 hoa don
CREATE OR REPLACE PROCEDURE WEB_RECALC_BILL_TOTAL(p_bill_id IN NUMBER)
IS
    v_tien_mon_an NUMBER;
    v_tien_giam NUMBER;
BEGIN
    ...
    UPDATE HoaDon
    SET TienMonAn = v_tien_mon_an,
        Tongtien = v_tien_mon_an - v_tien_giam
    WHERE ID_HoaDon = p_bill_id;
END;
/

-- Procedure tinh lai tien cho tat ca hoa don chua thanh toan
CREATE OR REPLACE PROCEDURE WEB_RECALC_OPEN_BILLS
IS
BEGIN
    FOR cur IN (
        SELECT ID_HoaDon
        ...
        WEB_RECALC_BILL_TOTAL(cur.ID_HoaDon);
    END LOOP;
END;
/

-- Index phuc vu truy van web (tao neu chua ton tai)
BEGIN
    EXECUTE IMMEDIATE 'CREATE INDEX IDX_HD_WEB_TABLE_STATUS ON HoaDon(ID_Ban, Trangthai)';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -955 THEN
            RAISE;
...
BEGIN
    EXECUTE IMMEDIATE 'CREATE INDEX IDX_HD_WEB_CUSTOMER_STATUS ON HoaDon(ID_KH, Trangthai)';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -955 THEN
            RAISE;
...
BEGIN
    EXECUTE IMMEDIATE 'CREATE INDEX IDX_CTHD_WEB_BILL ON CTHD(ID_HoaDon)';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -955 THEN
            RAISE;
...
BEGIN
    EXECUTE IMMEDIATE 'CREATE INDEX IDX_BAN_WEB_FLOOR_STATUS ON Ban(Vitri, Trangthai)';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -955 THEN
            RAISE;
...
BEGIN
    EXECUTE IMMEDIATE 'CREATE INDEX IDX_MONAN_WEB_TYPE_STATUS ON MonAn(Loai, TrangThai)';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -955 THEN
            RAISE;
...
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('loQy','20 off Aries',20,'Aries',10,200);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('pCfI','30 off Taurus',30,'Taurus',5,300);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('pApo','20 off Gemini',20,'Gemini',10,200);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('ugQx','100 off Virgo',100,'Virgo',3,500);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('nxVX','20 off All',20,'All',5,300);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('Pwyn','20 off Cancer',20,'Cancer',10,200);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('bjff','50 off Leo',50,'Leo',5,600);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('YPzJ','20 off Aquarius',20,'Aquarius',5,200);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('Y5g0','30 off Pisces',30,'Pisces',5,300);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('7hVO','60 off Aries',60,'Aries',0,1000);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('WHLm','20 off Capricorn',20,'Capricorn',0,200);
insert into Voucher(Code_Voucher,Mota,Phantram,LoaiMA,SoLuong,Diem) values ('GTsC','20 off Leo',20,'Leo',0,200);


--Them data cho bang HoaDon
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (101,100,100,'10-1-2023',0,0,'Da thanh toan','cash',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (102,104,102,'15-1-2023',0,0,'Da thanh toan','card',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (103,105,103,'20-1-2023',0,0,'Da thanh toan','transfer',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (104,101,101,'13-2-2023',0,0,'Da thanh toan','cash',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (105,103,120,'12-2-2023',0,0,'Da thanh toan','card',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (106,104,100,'16-3-2023',0,0,'Chua thanh toan','cash','pending');
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (107,107,103,'20-3-2023',0,0,'Chua thanh toan','transfer',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (108,108,101,'10-4-2023',0,0,'Chua thanh toan',NULL,NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (109,100,100,'20-4-2023',0,0,'Da thanh toan','card',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (110,103,101,'5-5-2023',0,0,'Da thanh toan','cash',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (111,106,102,'10-5-2023',0,0,'Chua thanh toan','transfer','pending');
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (112,108,103,'15-5-2023',0,0,'Chua thanh toan',NULL,NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (113,106,102,'20-5-2023',0,0,'Da thanh toan','cash',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (114,108,103,'5-6-2023',0,0,'Chua thanh toan','card','pending');
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (115,109,104,'7-6-2023',0,0,'Chua thanh toan',NULL,NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (116,100,105,'7-6-2023',0,0,'Da thanh toan','transfer',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (117,106,106,'10-6-2023',0,0,'Chua thanh toan',NULL,NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (118,102,106,'10-2-2023',0,0,'Da thanh toan','cash',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (119,103,106,'12-2-2023',0,0,'Da thanh toan','card',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (120,104,106,'10-4-2023',0,0,'Chua thanh toan',NULL,NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (121,105,106,'12-4-2023',0,0,'Da thanh toan','transfer',NULL);
INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai,PaymentMethod,PaymentRequestStatus) VALUES (122,107,106,'12-5-2023',0,0,'Da thanh toan','cash',NULL);
...
-- Note: Bills have been pre-populated with payment statuses. Do not update all to 'Da thanh toan' to preserve payment request demo data.
-- Some bills have PaymentRequestStatus = 'pending' to showcase the admin payment approval workflow.
